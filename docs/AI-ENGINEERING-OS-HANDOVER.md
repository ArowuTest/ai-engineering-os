...
**Last updated:** 16 August 2026, subscription-execution reconciliation Tasks 4, 5 and 6 accepted; Task 7 thin harness-adapter integration is active next; OpenSandbox trial-provider architecture and ECC-reuse/Review-Council supersession decisions remain locked.
...
**Immediate next action:** Task 7 — inventory and reuse the inherited ECC Codex/Claude/multi-harness assets, then add thin Codex, Antigravity `agy`, and Claude Code adapters over the accepted runner/environment seams. Start with fake-process RED proofs for exact argv/cwd/env, `shell:false`, objective transport without shell interpolation, cancellation, malformed output, local-auth non-export and capability matching. Perform each controlled local live proof before changing its delegation policy, and commit each harness independently rather than as a three-harness mega-commit.
